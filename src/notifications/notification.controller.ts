...
export class NotificationController {
  ...
  async subscribe(@Request() req, @Body() subscription: PushSubscription) {
    try {
      ...
      await this.notificationService.saveSubscription(req.user.id, subscription);
  ...
  async unsubscribe(@Request() req, @Body() data: { endpoint: string }) {
    await this.notificationService.removeSubscription(req.user.id, data.endpoint);
    return {
      success: true,
  ...
  async getSubscription(@Request() req) {
    const subscriptions = await this.notificationService.getUserSubscriptions(req.user.id);
    return {
      success: true,
      data: {
  ...
  async sendTestNotification(@Request() req) {
    try {
      ...
      const result = await this.notificationService.sendPushNotification(req.user.id, {
  ...
  getVapidPublicKey() {
    const publicKey = process.env.VAPID_PUBLIC_KEY || 'BI8AJsdXH6j4GnERi-vfMif-R1BrTuLPTu2q-24fSq6yvotc6A6lMo1Nq2Sqk0PZUhSTxGHRBS4WObIT7at4xV4';
    return {
      success: true,
      data: {
        publicKey
