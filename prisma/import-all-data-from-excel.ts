...
function getPositionProperties(positionName: string): {
  ...
  if (
    pos === "ptgđ" ||
    pos === "phó tổng giám đốc" ||
    pos.includes("phó tổng giám đốc")
  ) {
  ...
  if (
    ...
    pos === "trưởng team" ||
    pos === "team leader" ||
    pos.includes("trưởng team") ||
    pos.includes("team leader") ||
    pos.includes("trưởng nhóm") ||
    pos.includes("group leader") ||
    pos === "t.line" ||
    pos === "trưởng line" ||
    pos === "line leader" ||
    pos.includes("trưởng line") ||
    pos.includes("line leader")
  ) {
  ...
  if (
    pos === "tt" ||
    (pos === "tổ trưởng" && !pos.includes("phó")) ||
    pos === "đt" ||
    pos === "đội trưởng" ||
    pos === "trưởng đội" ||
    pos.includes("đội trưởng") ||
    pos.includes("trưởng đội") ||
    pos === "tca" ||
    pos === "trưởng ca" ||
...
function generateEmail(msnv: string, hoTen: string): string {
  ...
  if (nameParts.length >= 2) {
    ...
    const firstLettersOfMiddle = middleNames
...
function convertSex(sexStr: string | undefined): Sex | undefined {
  ...
  if (
    normalized === "nữ" ||
    normalized.normalize("NFD") === "nữ".normalize("NFD")
  ) {
    return Sex.FEMALE;
  }
...
async function processExcelData(): Promise<ProcessedData> {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const dateOfBirth = row[10] ? formatDateOfBirth(String(row[10])) : null;
      const sex = row[11] ? convertSex(String(row[11]).trim()) : null;

      const email = generateEmail(msnv, hoTen);
    ...
    console.log(
      `   - Management relations: ${processed.managementRelations.length}`
    );
...
async function createOffices(offices: Set<string>): Promise<Map<string, any>> {
  ...
  for (const officeName of offices) {
    try {
      ...
      if (
        officeName.includes("VP") ||
        officeName.includes("Văn phòng") ||
        officeName.includes("VPĐH") ||
        officeName.includes("Điều hành")
      ) {
      ...
      console.error(
        `   ❌ Failed to create office ${officeName}:`,
        error.message
      );
    }
  }
...
async function createDepartments(
  ...
  for (const [deptKey, dept] of departments) {
    try {
      ...
      if (!office) {
        console.warn(
          `   ⚠️  Office not found for department: ${dept.name} (${dept.office})`
        );
        continue;
      }
      ...
      console.error(
        `   ❌ Failed to create department ${dept.name}:`,
        error.message
      );
    }
  }
...
async function createPositions(
  ...
  for (const positionName of positions) {
    try {
      ...
      const { isManagement, canViewHierarchy, level } =
      ...
      console.log(
        `   ✅ ${positionName} → ${description} (Level: ${level}, Management: ${isManagement}, CanView: ${canViewHierarchy}, Reportable: ${isReportable})`
      );
    } catch (error) {
      console.error(
        `   ❌ Failed to create position ${positionName}:`,
        error.message
      );
    }
  }
...
async function createJobPositions(
  ...
  for (const [jobKey, jp] of jobPositions) {
    try {
      ...
      if (!department) {
        console.warn(
          `   ⚠️  Department not found: ${jp.pb} (${jp.tt}) for job ${jp.vt}`
        );
        continue;
      }
      ...
      const code = `${jp.cd.replace(/\s+/g, "").toUpperCase()}_${jp.vt
        .replace(/\s+/g, "")
        .toUpperCase()}_${jp.pb.replace(/\s+/g, "").toUpperCase()}`;
      ...
      console.error(
        `   ❌ Failed to create job position ${jp.vt}:`,
        error.message
      );
    }
  }
...
async function createUsers(
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      ...
      if (existingUser) {
        console.warn(
          `   ⚠️  User already exists: ${employeeCode} - ${fullName}`
        );
      ...
      let email =
        userData.email || generateEmail(userData.msnv, userData.hoTen);
      let emailSuffix = 0;
      ...
      while (!isEmailUnique) {
        const emailToCheck =
          emailSuffix === 0
            ? email
            : email.replace("@tbsgroup.vn", `${emailSuffix}@tbsgroup.vn`);
      ...
      if (!jobPosition) {
        console.warn(
          `   ⚠️  Job position not found for user: ${employeeCode} - ${fullName} (${jobKey})`
        );
        errorCount++;
        continue;
      }
      ...
      if (!office) {
        console.warn(
          `   ⚠️  Office not found for user: ${employeeCode} - ${fullName} (${userData.tt})`
        );
        errorCount++;
        continue;
      }
      ...
      console.error(
        `   ❌ Error creating user ${userData.msnv} - ${userData.hoTen}:`,
        error.message
      );
      errorCount++;
    }
...
async function createManagementRelations(
  ...
  for (const relation of managementRelations) {
    ...
    if (!userWithJobPosition?.jobPosition?.department) {
      console.warn(
        `   ⚠️  Department not found for user: ${relation.userMsnv}`
      );
      errorCount++;
      continue;
    }
    ...
    for (const departmentId of departments) {
      try {
        ...
        const existingRelation =
          await prisma.userDepartmentManagement.findUnique({
            where: {
              userId_departmentId: {
                userId: manager.id,
        ...
        if (existingRelation) {
          console.warn(
            `   ⚠️  Management relation already exists: ${managerMsnv} -> Dept ${departmentId}`
          );
          skipCount++;
          continue;
        }
        ...
        console.log(
          `   ✅ ${managerMsnv} manages department: ${
            department?.name || departmentId
          }`
        );
      } catch (error) {
        console.error(
          `   ❌ Error creating management relation ${managerMsnv} -> ${departmentId}:`,
          error.message
        );
        errorCount++;
      }
  ...
  console.log(
    `   Total department management relations: ${managementStats.length}`
  );
  ...
  try {
    ...
    if (!connected) {
      ...
      if (nodeEnv === "production") {
        ...
        console.error(
          "   2. Verify database is running: flyctl status -a weekly-report-backend-db"
        );
        console.error(
          '   3. Test connection: flyctl ssh console -C "npx prisma db pull"'
        );
        console.error(
          "   4. Check database logs: flyctl logs -a weekly-report-backend-db"
        );
      }
    ...
    console.log(
      `   🔗 Management Relations: ${processedData.managementRelations.length} processed`
    );
